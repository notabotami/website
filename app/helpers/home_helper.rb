module HomeHelper



  ######

  def self.en_thanks
    return "Thank you for visiting our website!"
  end

  def self.es_thanks
    return "Muchas gracias por visitar nuestra p&aacutegina de internet!"
  end

  ######

  def self.en_para1
    return "48th Street Income Tax Service is one of the fastest growing income tax preparation providers in Denison Cities and surrounding areas."
  end

  def self.es_para1
    return "48th Street Income Tax Service es uno de los proveedores de
preparaci&oacuten de impuestos con mayor crecimiento en los &uacuteltimos a&ntildeos en la
ciudad de Denison Iowa ys us alrededores"
  end

  #####

  def self.en_para2
    return "We understand that preparing your own federal income taxes can be complicated and
  confusing.  Our trained professionals are here to help you with all of your tax
  concerns.  We are commited to providing you with excellent customer service."
  end

  def self.es_para2
    return "Nosotros entendemos que el proceso de preparar sus planillas
de impuestos puede ser complicado y confuso.  Nuestros profesionales
estan dispuestos para ayudarle con todas sus inquietudes acerca del
proceso tributario de los Estados Unidos.  Estamos comprometidos a
proporcionarle nuestro servicio de atencion al cliente de la m&aacutes alta calidad"
  end

  #####

  def self.en_para3
    return "At 48th Street Income Tax Service we specialize in preparing tax returns through
  electronic filing (IRS e-file) so you can recieve your tax return even faster.
In addition to preparing federal and any state
  individual and business tax returns, 48th Street Income Tax Service provides translation
and interpretation services as well as notary services."
  end

  def self.es_para3
    return "En 48th Street Income Tax Service nos especializamos en la
preparaci&oacuten de declaraciones de impuestos individuales y de negocios a trav&eacutes de
la presentatci&oacuten electronica (e-file del IRS) para que usted pueda
recibir su reembolso aun mas rapido.  Ademas de preparar impuestos federales y cualquier
declaraci&oacuten de impuesto individuale estatale, 48th Street Income Tax Service ofrece
servicios de traducci&oacuten y interpretacci&oacuten, tambien servicios de notarizaci&oacuten
de documentos."
  end

  #####

  def self.en_para4
    return "No matter what your occupation, our tax professsionals are experts in the field of
  tax preparation.  We are extremely knowledgable about the latest tax law
  changes and strive to keep up with the ever changing tax code."
  end

  def self.es_para4
    return "No importa cual sea su ocupacion, nuestros profesionales
son expertos en el campo de la preparacion de impuestos.  Estamos
muy bien informados acerca de los ultimos cambios en la ley de los
impuestos tributarios y nos esforzamos para mantenernos al dia
en los constantes cambios de los codigos tributarios"
  end

  #####

  def self.en_para5
    return "At 48th Street Income Tax Service, you receive personalized service from every one
  of our tax professionals.  We offer one-on-one interviews to identify all the
  deductions you are entitled to, and we work to get you the largest refund possible.
"
  end

  def self.es_para5
    return "En 48th Street Income Tax Service, usted recibe un servicio
personalizado de cada uno de nuestros profesionales en la preparacion
de impuestos.  Ofrecemos entrevistas cara-a-cara durante la preparacion
de sus impuestos para poder identificar todas las deducciones sobre
las cuales usted tiene derecho, y trabajamos para que usted obtenga
el mayor reembolso posible bajo las leyes tributarias."
  end

  #####

  def self.en_offer_header
    return  "We offer the following services:"
  end

  def self.es_offer_header
    return "Le Ofrecemos los siguientes servicios:"
  end

  #####

  def self.en_offer1
    return "Professional Income Tax Preparation"
  end

  def self.es_offer1
    return "Preparacion de Impuestos Profesional"
  end

  #####

  def self.en_offer2
    return "Ammendments on Returns"
  end

  def self.es_offer2
    return "Enmiendas en sus Planillas de Impuestos"
  end

  #####

  def self.en_offer3
    return "Free Electronic Filing (IRS e-file)"
  end

  def self.es_offer3
    return "Presentacion Electronica Gratis (e-file del IRS)"
  end

  #####

  def self.en_offer4
    return "Free Direct Deposit"
  end

  def self.es_offer4
    return "Deposito Directo Gratis a su Cuenta Bancaria"
  end

  #####

  def self.en_offer5
    return "Electronic Refund Checks (ERCs)"
  end

  def self.es_offer5
    return "Cheques de Reembolso Electronico (ERCs)"
  end

  #####

  def self.en_offer6
    return "Income Tax Planning"
  end

  def self.es_offer6
    return "Planification y Asesoria sobre los Impuestos"
  end

  #####

  def self.en_offer7
    return "Translations and Interpretation Services (English and Spanish)"
  end

  def self.es_offer7
    return "Servicio de Traducciones e Interpretacion (Ingles y Espa&ntildeol)"
  end

  #####

  def self.en_offer8
    return "Notary Services"
  end

  def self.es_offer8
    return "Servicios de Notarizacion de Documentos"
  end


end